...
public abstract class JEMGrader
{
  ...
  /** Compiles and runs code in each student folder
    *
    * 1. Creates a temp folder to do work in
    * 2. Calls test on each folder in the student folder
   ...
   *
   *  1. Creates a temp folder inside the specified temp folder (which makes that a folder of temp folders)
   *  2. Copies all files from the specified testDir into the temp folder
   *  3. Copies all student files from the specified student folder
   *  4. Compiles the code
   *  5. Runs the code
   *  6. Deletes the temp folder
   *
   *  If there is a compile error, then a compile.log file is created in the student folder that contains the Ststem.err stream
   *  If there is no compile error, the an output.log file is created in the student folder taht contains the System.out stream
   */
  private void test(File studentDir, File testDir, File tempFolderFolder)
  {
    ...
    String tempDirName = tempFolderFolder.getPath()+File.separator+"temp-"+studentDir.getName()+"-"+System.currentTimeMillis();
